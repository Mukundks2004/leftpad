...
theorem MyList.is_suffix_of_itself {α : Type} [MyEq α] (xs : MyList α) : xs.is_suffix_of xs = MyBool.my_true := by
  induction xs with
  | my_nil => rfl
  | my_cons hd tl ih =>
    rw [MyList.is_suffix_of, MyEq.my_eq_refl hd, ih, MyBool.and_true_left]

theorem MyList.is_suffix_after_appending {α : Type} [MyEq α] (xs ys : MyList α) : xs.is_suffix_of (xs.my_append ys) = MyBool.my_true := by
  induction xs with
  | my_nil =>
...
theorem MyList.reverse_is_suffix_after_reversed_prepend {α : Type} [MyEq α] (xs ys : MyList α) : (MyList.my_reverse xs).is_suffix_of (MyList.my_reverse (ys.my_append xs)) = MyBool.my_true := by
  rw [MyList.append_reverse, MyList.is_suffix_after_appending]

theorem MyList.is_prefix_after_prepending {α : Type} [MyEq α] (xs ys : MyList α) : xs.is_prefix_of (ys.my_append xs) = MyBool.my_true := by
  rw [MyList.is_prefix_of, MyList.reverse_is_suffix_after_reversed_prepend]
