...
theorem MyNat.succ_add (m n : MyNat) : m.my_succ.my_add n = (m.my_add n).my_succ := by
  ...
  | my_zero => rfl
  | my_succ n ih =>
    rw [MyNat.my_add, ih, MyNat.add_succ]

theorem MyNat.add_comm (m n : MyNat) : m.my_add n = n.my_add m := by
  induction m with
  | my_zero => rw [MyNat.zero_add, MyNat.add_zero]
  | my_succ m ih =>
    rw [MyNat.my_add, MyNat.succ_add, ih]

/- Subtraction properties -/
theorem MyNat.zero_sub (m : MyNat) : MyNat.my_zero.my_sub m = MyNat.my_zero := by
  match m with
    | MyNat.my_zero => rfl
    | MyNat.my_succ _ => rfl
...
theorem MyNat.sub_add_eq_max (m n : MyNat) : (m.my_sub n).my_add n = m.my_max n := by
  ...
  | MyNat.my_succ m, MyNat.my_succ n =>
    rw [MyNat.my_sub, MyNat.my_max, MyNat.my_add, MyNat.sub_add_eq_max m n]
